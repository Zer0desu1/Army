...
const SWING = Math.PI / 7;          // walk swing angle
const HEAD_S = 0.30;                // head cube half-size
...
export class DigitalTwinComponent implements AfterViewInit, OnDestroy, OnChanges {
  ...
  // Bir harita-birimi (≈ metre) için kaç adım kullanılacak.
  // 1 adım ≈ 250ms sürüş; STEPS_PER_UNIT artarsa robot aynı tıklamada daha uzun yol gider.
  ...
  /* ── Bubble (collision radius per robot) ── */
  ...
  // Wheel/Track parts for other robots
  /* ── Materials (reused) ── */
...
ngOnDestroy() {
  ...
  private buildRobot() {
    ...
    // Drop positions for removed robots
  ...
  // Hedef pozisyona göre uygun komutu ve mesafeye orantılı adım sayısını hesapla.
  // Konvansiyon (onMapMove ile aynı): walk = -z (ileri), back = +z (geri).
  // Omni robotlar için yan komutlar da seçilebilir: strafeLeft/strafeRight.
  // Diğerleri (dog/tank) için sadece walk/back döner.
  // Adım sayısı: kullanılan ekseni dominant-axis projeksiyonu uzunluğundan türetilir.
  private planMove(id: string, x: number, z: number): { cmd: string, steps: number } {
    ...
    // Adım sayısı: mesafe × katsayı, alt/üst sınır
  ...
  public goToWaypoint(wp: {id: string, x: number, z: number}) {
    ...
    targets.forEach((r, idx) => {
      const id = r.id;
      if (!id) return;
      // Spread broadcast targets in a small ring around the waypoint
...
/* ═══════════════════════════════════════════════════════
  ...
  private buildOmni3(state: RobotState): TwinInstance {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      wheelGrp.rotation.y = angle + Math.PI/2; // local X points to center
  ...
  private buildOmni4(state: RobotState): TwinInstance {
    ...
    const yellowMat = new THREE.MeshStandardMaterial({ color: 0xfacc15, roughness: 0.5, metalness: 0.3 }); // Yellow plate
    const blackMat = new THREE.MeshStandardMaterial({ color: 0x111111, roughness: 0.8 });
    ...
    // 1. Main Square Plate (Yellow)
    // The photo shows it rotated 45 degrees relative to the wheel axes, or the wheels are at the corners.
    // Let's make a square plate, and place wheels at the corners, rotated 45 deg.
    ...
    // Bolts on the plate (for visual detail)
    ...
    // 2. Side Panel with Switch (underneath, on one side)
    ...
    // 3. 4 Omni Wheels and Motors at corners
    const wheelPositions = [
      { x: -0.45, z:  0.45, angle: -Math.PI/4 },     // Front-Left
      { x:  0.45, z:  0.45, angle: Math.PI/4 },      // Front-Right
      { x: -0.45, z: -0.45, angle: -Math.PI*3/4 },   // Back-Left
      { x:  0.45, z: -0.45, angle: Math.PI*3/4 }     // Back-Right
    ];

    for (let i = 0; i < 4; i++) {
      const pos = wheelPositions[i];
      
      // Motor (Black cylinder)
      ...
      // Orient the wheel's local X axis to be the axle direction
  ...
  private buildEnvironment() {
    const R = 8; // expanded ground radius

    // Circular platform - Lighter color to separate from robot
    ...
    // Neon outer ring around platform
    ...
    // Inner concentric rings (decorative)
    ...
    // Grid on platform - Lighter lines
    ...
    // Corner accents around larger area
    for (let i = 0; i < 8; i++) {
  ...
  private animate() {
    ...
    for (const inst of this.instances) {
      ...
      // Move toward target while steering around other robots' bubbles
      if (inst.id) {
          ...
          if (dist > 0.05) {
            ...
            for (const other of this.instances) {
              ...
              if (d < lookAhead) {
                ...
                // Project out the component going INTO the other robot
                const dot = vx * nx + vz * nz;
                if (dot < 0) { vx -= dot * nx; vz -= dot * nz; }
                // Hard separation if already overlapping
                if (d < minSep) {
            ...
            // Head-on deadlock: if steering zeroed velocity, take a tangent around nearest
